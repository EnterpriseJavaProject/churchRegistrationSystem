...
if (isset($_POST['check_submit_btn']))
 ...
 if (isset($_POST['register_btn']))
 {
  ...
  $membershipID = $_POST['membershipNo'];
...
$checkMemebr = "SELECT membershipNo FROM members ORDER BY membershipNo DESC";
$checkMemebrResult = mysqli_query($connection,$checkMemebr);

$row = mysqli_fetch_array($checkMemebrResult);

$membershipID = $row['membershipNo'];
...
if (empty($membershipID)) {
  ...
  $get_numbers = str_replace("HGT", "", $membershipID);
  $get_string = str_pad($get_numbers + 1, 3, 0, STR_PAD_LEFT);
  $membershipNo = 'HGT' .$get_string;

}



$query = "INSERT INTO members (membershipNo, username, phone, gender, marital_status, dob, occupation, place_of_work, residence, department, tithe, year_baptised,home_town,region, country, usertype, `images`) VALUES ('$membershipNo', '$username','$phone','$gender', '$marital_status', '$dob', '$occupation','$place_of_work','$residence','$department','$tithe','$year_baptised', '$home_town', '$region', '$country', '$usertype', '$images')";
      ...
      if ($query_run) 
        {
          ...
          $_SESSION['status'] = $username. " Added Successfully.  Membership No : " .$membershipNo;
